...
$seed     = Join-Path $OutputRoot $SeedListName
$manifest = Join-Path $ContentFolder $ManifestName
$failures = Join-Path $ContentFolder $FailuresName
if (-not (Test-Path $seed)) { throw "Missing $seed. Run 10_FindKB.ps1 first." }
...
$prev = @{}; Import-Csv $manifest | % { $prev[$_.Url] = $_ }

Get-Content $seed | ? { $_ -and $_ -notmatch '^\s*#' } | % {
  $norm = Normalize-Url $_; try { $u=[uri]$norm } catch { return }
  if (-not (Test-AllowList -Uri $u -AllowedHosts $AllowedHosts -InfoPathPrefix $InfoPathPrefix)) { return }
